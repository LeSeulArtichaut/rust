...
impl<'tcx> LayoutCx<'tcx, TyCtxt<'tcx>> {
    ...
    fn generator_layout(
        ...
        let tag_layout = tcx.intern_layout(Layout::scalar(self, tag.clone()));
        ...
        + HasParamEnv<'tcx>,
{
    ...
    fn field(this: TyAndLayout<'tcx>, cx: &C, i: usize) -> C::TyAndLayout {
        ...
        cx.layout_of(match *this.ty.kind(tcx) {
            ...
            ty::Ref(_, pointee, _) | ty::RawPtr(ty::TypeAndMut { ty: pointee, .. }) => {
                ...
                match tcx.struct_tail_erasing_lifetimes(pointee, cx.param_env()).kind(cx.tcx()) {
    ...
    fn pointee_info_at(this: TyAndLayout<'tcx>, cx: &C, offset: Size) -> Option<PointeeInfo> {
        let addr_space_of_ty = |ty: Ty<'tcx>| {
            if ty.is_fn(cx.tcx()) { cx.data_layout().instruction_address_space } else { AddressSpace::DATA }
        };

        let pointee_info = match *this.ty.kind(cx.tcx()) {
            ...
            _ => {
                ...
                if let Some(ref mut pointee) = result {
                    if let ty::Adt(def, _) = this.ty.kind(cx.tcx()) {
        ...
        call::FnAbi::new_internal(cx, sig, extra_args, caller_location, attrs, |ty, arg_idx| {
            ...
            if let (ty::InstanceDef::Virtual(..), Some(0)) = (&instance.def, arg_idx) {
                let fat_pointer_ty = if layout.is_unsized() {
                    ...
                    'descend_newtypes: while !fat_pointer_layout.ty.is_unsafe_ptr(cx.tcx())
                        && !fat_pointer_layout.ty.is_region_ptr(cx.tcx())
                    {
