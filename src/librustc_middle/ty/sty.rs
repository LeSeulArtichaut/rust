...
impl<'tcx> TyS<'tcx> {
    #[inline(always)]
    pub fn kind(&self, _tcx: TyCtxt<'tcx>) -> &TyKind<'tcx> {
        &self.kind
    }
    ...
    #[inline]
    pub fn is_param(&self, tcx: TyCtxt<'tcx>, index: u32) -> bool {
        match self.kind(tcx) {
    ...
    pub fn simd_size(&self, tcx: TyCtxt<'tcx>) -> u64 {
        // `tcx` probably needed in the future to allow
        // `#[repr(simd)] struct Simd<T, const N: usize>([T; N]);`.
        match self.kind(tcx) {
    ...
    pub fn builtin_deref(&self, tcx: TyCtxt<'tcx>, explicit: bool) -> Option<TypeAndMut<'tcx>> {
        match self.kind(tcx) {
